...
#if ALL(ANYCUBIC_SOFT_TFT_MODEL_OLD, EXTENSIBLE_UI, ANYCUBIC_TFT_SERIAL)
...
{
  uint16_t i = (uint16_t)(buffer->head + 1) % SERIAL_BUFFER_SIZE;
...
{
  uint16_t i = (uint16_t) (_tx_buffer->head + 1) % SERIAL_BUFFER_SIZE;
